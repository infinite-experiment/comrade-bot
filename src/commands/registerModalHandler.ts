...
export async function execute(interaction: DiscordInteraction) {
    ...
    try{
        const initRegistration = await ApiService.initiateRegistration(interaction.getMetaInfo(), ifcId, lastFlight);
        await interaction.reply(MessageFormatters.makeRegistrationString(initRegistration));
        return;
    } catch(err) {
        console.log(err);
    }
